...
@{Makefile::EXPORT_OK} = qw/get_version path_copy file_copy rmdir_force dir_copy pd_read pd_write get_version_from compare_version/;
...
sub new {
	...
	$self->print_log_header("Script: $0");
...
sub check_manifest {
	...
	for ($self->manifest) {
		...
		unless (-f $_) {
			...
			$self->print_log( "The file \"$_\" seems to miss from your package.", 'error');
		}
	}
	if ($self->{manifest_ok}) {
		print "Manifest seems to be ok.\n";
		$self->print_log("Manifest seems to be ok.", "CHECK");
		return 1;
	}
	else { 
...
sub check_dep {
	...
	while (my ($module, $version) = each %{$self->{include}{PREREQ_PM}}) {
		...
		unless (compare_version($version, $our_v) >= 0) {
			$self->{dep_ok} = 0;
			print STDERR "==> Failed dependency \"$module\"".($version ? " version $version\n" : "\n");
			$self->print_log("Failed dependency \"$module\"".($version ? " version $version" : ""), 'error');
		}
	}
	if ($self->{dep_ok}) {
		print "Dependencies seem to be ok.\n";
		$self->print_log("Dependencies seem to be ok.", "CHECK");
		return 1;
	}
	else {
...
END
	;
	for (keys %{$self->{vars}}) { print '  '.$_.($self->{vars}{$_} ? ' :  '.$self->{vars}{$_} : '')."\n"; }
	print "\n\n";
}
...
sub truncate_log {
	...
	my $version = $self->{include}{VERSION} || 'unknown';
	my $author = $self->{include}{AUTHOR} || 'unknown';
	if ($self->{log_file}) { #print "log file: $self->{log_file}\n";
		open L, '>'.$self->{log_file} || die $!;
		print L << "END"
# Log file for the make/compile/test/install process
...
sub print_log {
	...
	my $text = shift;
	my $type = shift || 'MESS';
	if ($self->{log_file}) { #print "debug got: ".uc($type).": $text -- for log file $self->{log_file}\n";
		unless (-f $self->{log_file}) { die "Could not find log file" }
		open L, '>>'.$self->{log_file} || die $!;
		print L uc($type).": $text\n";
		close L;
	}
}

## helpfull class routines
...
sub get_version_from {
	...
	open PACK, $f || die $!;
	while (<PACK>) {
		if(/([\$*])(([\w\:\']*)\bVERSION)\b.*\=/) { # regex from MakeMaker docs
			$version = eval($_);
			last;
		}
	}
...
sub file_copy {
    ...
    if (([stat$from]->[9]==[stat$to]->[9])&&(-s$from==-s$to)) {
        return 1;
    }
...
sub rmdir_force {
	...
	closedir D;
	for (grep { !(-d $dir.'/'.$_) } @dinge) { unlink $dir.'/'.$_ || die $! }
...
Log file for the make process, defaults to F<m/make.log>.
If empty string no log is created.
...
This hash defaults to C<{ 'PERL' => [ '/usr/bin/perl', ['pl', 'PL', 'pm'] ], 'SHELL' => [ '/bin/sh', ['sh'] ], }>
...
Also this method truncates the log file when called from F<Makefile.PL>.

=item pd_read($file)

Eval $file and return $VAR1 .

=item pd_write($file, \%hash)
...
=item print_log($text, $type)

Print entry to log file (if there is one) of the format C<TYPE: text>.

=item compare_version($v1, $v2) [non oo method]

Returns 0 if $v1 eq $v2, 1 if $v1 < $v2
and -1 if $v1 > $v2.
Used to compare required versions to installed versions.

=item get_version($class_name) [non oo method]

Get version of an installed module.

=item get_version_from($file) [non oo method]
...
=item path_copy($from, $to) [non oo method]

Copy file $from to file $to, create directories if needed.

=item file_copy($from, $to) [non oo method]

Copy file $from to file $to, create directories if needed.

=item rmdir_force($dir) [non oo method]

Like C<rm -fr dir>, used to rm non-empty dir trees.
I<Use with care>.

=item dir_copy($from, $to) [non oo method]
...
It depends on other modules -- like Data::Dumper. Also the scripts use other
...
Well, next time ask yourself, are these docs comprehensible, complete and simple enough for the average coder?
Also one example might do more good then any number of lines description.
