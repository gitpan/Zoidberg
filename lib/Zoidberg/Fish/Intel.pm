...
sub expand {
	...
	unless ($context) { $context = $tree->[-1][2]; } # extreem vunzig
...
sub expand_files { # lousy hack
	...
	my @results = ();
	$self->flush;
	if (-d (my $dir = abs_path($string)) ) { @results = ($dir); }
	else {
...
sub c_system {
	...
	if (defined $set) {
		if ($set->{space_blocks} > 0) {
			if ($set->{old_arg} =~ m/^-/) { $self->parse($set, 'MAN_ARGS'); }
...
sub c_path {
	...
	if (defined $set) {
		push @{$set->{poss}}, sort grep /$set->{arg}/, @{&list_path};
		$set->{postf} = ' ';
		push @{$self->{poss}}, $set;
	}
}
...
sub c_file {
	...
	if ( defined ($set = $self->set_arg($set, $2)) ) { #print "debug: file: arg: $set->{arg}\n";
		...
		$set->{postf} = '';
		if ($self->{config}{hide_hidden_files} && ($set->{arg} !~ /^\^\./)) {
			@{$set->{poss}} = grep {$_ !~ /^\./} @{$set->{poss}};
		}
		push @{$self->{poss}}, $set;
	}
}
...
sub c_man_args {
	...
	if (defined $set) {
		...
		if (open MAN, $self->parent->{core}{utils}{man}." ".$set->{tree}[0][0]." 2>/dev/null |") {
			my %info;
			while (<MAN>) {
				chomp;
				$_ =~ s/\e.*?m//g; # get rid of ansi colors
				s{^((?:\s*$set->{arg}(?:[\w-\(\)\=]+)\,?)+)(.*?)}{
					my $s = $1; #print "found: >>$1<<\n";
					$s =~ s/^\s*(.*)\s*$/$1/;
					my @a = split(/,?\s+/, $s);
					push @{$set->{poss}}, @a;
					if ($3) {for (@a) { $info{$_} = $3 } };
...
sub c_zoid {
	...
	if ( $pref && defined ($set = $self->set_arg($set, $arg)) ) {
		...
		unless (ref($ding)) {
			...
			if (($class eq ref($self->parent)) && (!$ding->{core}{show_naked_zoid}||$pref =~ /\xA3/)) {
				my $obj_set = dclone $set;
				push @{$set->{poss}}, sort grep {/$set->{arg}/} @{$self->parent->list_clothes};
				push @{$set->{poss}}, sort grep {/$set->{arg}/} @{$self->parent->list_vars};
				...
				no strict 'refs';
				push @{$set->{poss}}, grep {/$set->{arg}/} map {s/^(.+\:\:)*//g; $_} (Devel::Symdump->new($class,@{$class."::ISA"})->functions);
				$set = $self->cut_double($set);
				@{$set->{poss}} = sort @{$set->{poss}};
				push @{$set->{poss}}, sort grep {/$set->{arg}/} map {'{'.$_.'}'} keys %{$ding};
			}
		}
		if ($self->{config}{hide_private_methods} && ($set->{arg} !~ /^\^(\\\{)?_/)) {
			@{$set->{poss}} = grep {$_ !~ /^\{?_/} @{$set->{poss}};
		}
		push @{$self->{poss}}, $set;
	}
...
sub c_perl {
	...
	if ( defined ($set = $self->set_arg($set, $arg)) ) {
		my $block = $self->{block};
		$block =~ s/([\$\%\@\&]?)(\w*)$//;
		unless ($sig && ($sig ne '&')) { # perlfunc or subroutine
			my @subs = ();
			$block =~ s/sub\s+(\w+)/push @subs, $1;/ge;
			push @{$set->{poss}}, grep {/$set->{arg}/} @{$self->{parent}->{grammar}{perl_functions}};
			if ($sig eq '&') { 
				push @{$set->{poss}}, sort grep {/$set->{arg}/} @subs; 
				$set->{postf} = '(';
			}
			else { $set->{postf} = ' '; }
			push @{$self->{poss}}, $set;
			...
			my %postf = ( '$' => ' ', '%' => '{', '@' => '[' );
			for ( ($sig eq '$') ? ('$', '%', '@') : ($sig) ) {
				my $my_set = dclone $set;
				push @{$my_set->{poss}}, sort grep {/$my_set->{arg}/} @{$vars{$_}};
				$my_set->{postf} = ($sig eq '$') ? $postf{$_} : ' ';
				push @{$self->{poss}}, $my_set;
			}
		}
	}
}
...
sub c_hist { #print "debug: hist expand\n";
	...
	if ( defined ($set = $self->set_arg($set, $self->{old_block})) ) {
		push @{$set->{poss}}, grep {/$set->{arg}/} @{$self->parent->History->list_hist};
