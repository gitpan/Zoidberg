		...
		if (WIFSTOPPED($status)) {
			...
			if ($pid == $$self{procs}[-1][0]{pid}) { # the end of the line ..
				...
				unless ($self->completed) {
					...
					CORE::kill( 13 => -$$self{pgid} ); # SIGPIPE
				}
			}
		}
	}
...
sub _run { # TODO something about capturing :(
	...
	# do redirections and env
	my $save_fd = $self->_do_redirect($block);
	my $save_env = $self->_do_env($block);

	# here we go !
	eval { $self->{shell}{eval}->_eval_block($block) }; # VUNZig om hier een eval te moeten gebruiken

	# restore file descriptors and env
	$self->_undo_redirect($save_fd);
	$self->_undo_env($save_env);
