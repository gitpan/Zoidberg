...
sub parse {
	...
	if ($self->{collection}{$self->{last}}{rules}) { $self->parse_rules; }
...
sub stringify {
	...
	foreach my $block (@{$self->{tree}}) {
		...
		if ( ($block->[2]) && (grep {$block->[2] =~ /^$_$/i} @{$self->{collection}{ansi_colors}}) ) {
...
sub gen_rules {
	my $self = shift;
	if ($self->{collection}{$self->{last}}{syntax}) {
		foreach my $color (keys %{$self->{collection}{$self->{last}}{syntax}}) {
			my $exp = '('.join('|', @{$self->{collection}{$self->{last}}{syntax}{$color}}).')';
			unshift @{$self->{collection}{$self->{last}}{rules}}, ['^'.$exp.'$', '$block->[2] = "'.$color.'"; $lock++'];
		}
	}
}
