...
sub trog {
    ...
    for (@pending) {
        ...
        if ($self->{exec_error} ?
            ...
            $return = $self->scuddle($tree,$sign,$string);
        }
    }
    $self->update_status;
    return $return;
...
sub jobs {
    my $self = shift;
    map { if ($_->{foreground}) { $b=" &" } else { $b="" } $a="[$_->{id}] $_->{string} $b"; $self->print($a,'message');$a}  grep {ref!~/native/i} @{$self->{jobs}};
}
...
sub classify {
    my $self = shift;
    if (($#{$self->{tree}}==0)&&($self->{tree}[0][2]ne'SYSTEM')&&$self->{foreground}) {
    ...
    foreach my $job (@{$self->zoid->{jobs}}) {
        for (@{$job->{procs}}) {
            ...
            if (WIFSTOPPED($status)) {
                $self->{stopped} = 1;
                $self->zoid->print("Job stopped: $self",'message');
                return 1;
            }
            $_->{completed} = 1;
            if (WIFSIGNALED($status)) {
                $self->zoid->print("[$job->{id}] terminated by signal ".WTERMSIG($status),'message');
                $self->zoid->{exec_error} = 1;
            }
            elsif ($status) {
                $self->zoid->{exec_error} = 1;
