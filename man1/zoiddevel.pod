...
update routines; since 'postcmd' is called more often then 'prompt', use 'prompt' if it
makes no difference, 'postcmd' should only be used for atomic updates.

Note this event is _not_ identical to the "special alias" 'postcmd' in tcsh(1).
...
In secundairy scripts it is better to use C<< Zoidberg::Shell->current() >>.
