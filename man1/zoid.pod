...
Apart from the standard variables like $HOME and $USER, B<zoid> uses the 
...
Defines the directory to write temporary files, it defaults to F<$HOME/.cache>.

=item $ZOID

Contains the filename by which B<zoid> was executed. For 'sh-shells' this would
be $SHELL, but due to this some programs expect $SHELL contain the path to a fully
POSIX compliant shell.

=item $ZOIDPID

Contains the process id of the current B<zoid>, used for IPC.

=item $ZOIDREF

Contains either a reference or a string representation of the current Zoidberg 
object, used for IPC.
...
to check the search paths in use for a particular installation.
...
options are wrong. If the error was thrown by one of the core modules, the error 
...
package. Visit the web page to submit bug reports or mail the author.
