...
You can also bind command sequences to a key like this:
  ...
  ctrl_b => '->shell(q{perl Makefile.PL && make && make test install})'
...
After restarting zoid, you should have a builtin command "hello_world" that prints the string "Hello cruel world !", and an object called "HelloWorld".
Of course you guessed already that you can control the string that will be printed from the config file, and also that it defaults to "Hello world !".
On run-time this string is available under '->{settings}{HelloWorld}{string}', it can be changed at any time.
